...
def guardar_snapshot(cartera_id: int, ccl: float) -> dict:
    ...
    try:
        ...
        # Convertir a tipos Python nativos (evita np.float64 que rompe PostgreSQL)
        valor_usd = float(res.get("Valor actual (USD)") or 0)
        costo_usd = float(res.get("Costo total (USD)") or 0)
        gan_usd   = float(res.get("Ganancia total (USD)") or 0)
        gan_pct   = float(res.get("Ganancia total (%)") or 0)
        n_pos     = int(res.get("Posiciones") or 0)

        # Fallback: si valor_usd es 0 o None (Yahoo Finance bloqueado), usar ultimo snapshot valido
        if not valor_usd or valor_usd == 0:
            try:
                ...
                if not df_ult.empty:
                    ...
                    n_pos     = int(float(df_ult.iloc[0]["n_posiciones"] or 0))
            except Exception:
                pass

        # Si aun no hay valor valido, no guardar snapshot
        if not valor_usd or valor_usd == 0:
            return {"error": "Sin datos de precio disponibles. Snapshot no guardado.", "skip": True}
